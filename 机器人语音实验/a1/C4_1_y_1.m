...
[voiceseg,vsl,SF,NF,amp,zcr, amp2, amp1, zcr2]=vad_TwoThr(x,wlen,inc,NIS);  % 端点检测
subplot 312
...
%     nxl=voiceseg(k).duration;
    ...
    line([0 4],[amp2 amp2],'color','g','LineStyle','-');
    line([0 4],[amp1 amp1],'color','g','LineStyle','-');
    subplot 313
    ...
    line([0 4],[zcr2 zcr2],'color','g','LineStyle','-');
%     line([0 4],[0.7 0.7],'color','g','LineStyle','-');
end
h = sgtitle(['分帧参数：wlen=', num2str(wlen), ',inc= ', num2str(inc)]);
set(h, 'FontSize', 14, 'Color', 'blue','FontName', 'Seime')
gtext(['amp2=', num2str(amp2)], 'Color', 'red')
gtext(['amp1=', num2str(amp1)], 'Color', 'red')
gtext(['zcr2=', num2str(zcr2)], 'Color', 'red')
