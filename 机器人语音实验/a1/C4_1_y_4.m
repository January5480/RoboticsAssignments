...
[xx,fs]=audioread('C4_1_y.wav');                   % 读入数据
...
[voiceseg,vsl,SF,NF,Epara, T1, T2]=vad_pro(x,wnd,inc,NIS,th1,th2,mode);     %比例法
fn=length(SF);
...
for k=1 : vsl                         
    ...
    line([0, 4],[T1, T1], 'color','g');
    line([0, 4],[T2, T2], 'color','g');
end
h = sgtitle(['分帧参数：wlen=', num2str(wlen), ',inc= ', num2str(inc), '  模式选择：', num2str(mode)]);
set(h, 'FontSize', 14, 'Color', 'blue','FontName', 'Seime')
gtext(['T1=', num2str(T1)], 'Color', 'red')
gtext(['T2=', num2str(T2)], 'Color', 'red')
