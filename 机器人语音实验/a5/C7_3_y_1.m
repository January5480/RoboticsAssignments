...
[xx,fs]=audioread('C7_3_y.wav');                     % 读取文件
...
% % 用修改后的LPC内插法基音检测函数
% [voiceseg,vosl,SF,period]=pitch_LPC_Interpolation(x,wlen,inc,T1,fs,p);
% Dpitch=pitfilterm1(period,voiceseg,vosl);  % 平滑基音周期

%% 共振峰提取
for i=1:length(SF)
    [Frmt(:,i),Bw(:,i),U(:,i)]=Formant_Root(X(:,i),p,fs,3);  % LPC求根法
%      [Frmt(:,i),Bw(:,i),U(:,i)] = Formant_LPCC(X(:,i),p,fs,3);  % LPC内插法
% %      [Frmt(:,i),Bw(:,i),U(:,i)] = Formant_Cepstrum(X(:,i),p,fs,3);  % 倒谱法
...
figure;
N_fft = 1024;
subplot(2,1,1); spectrogram(x1, wind, overlap, N_fft, fs); title('原始语音语谱图');
subplot(2,1,2); spectrogram(output, wind, overlap, N_fft, fs); title('合成语音语谱图');
