...
// 如果顾客休息时间来，顾客正常入队，但是都要去零号窗口排队
// 不足之处在于，当正常上班之后，排在0号窗口的人不能更换队伍
// 由于队列先进先出的性质，很难让顾客再去换队伍
// 完成啦！！！！~~~~~
#include <cstdlib>
#include <ctime>
#include <iostream>
    ...
    for (int now = 0; now < service_duration + 1; ++now){ // 时间流�?
        ...
        if (bank->customer_queue.size > 0){ // 如果还有剩余顾客
            ...
            if (cur_customer.arrival_time == now) { // 该顾客已到达
                // dequeue(&bank->windows[i].queue);！！！！！
                // if(cur_customer.arrival_time > 330 && cur_customer.arrival_time < 210){
                //     NUM_WINDOWS = 1;
                // }
...
void EventDriven(Bank* bank, Customer* cur_customer, int now)
{
    // 找到最短服务窗口队列?
    int min = 0;
    if(cur_customer->arrival_time >= 210 && cur_customer->arrival_time <= 330){
        min = 0;
    }
    else{
