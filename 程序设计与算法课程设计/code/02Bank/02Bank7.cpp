// 实现一周工作6天，周日不上班,中午休息，无值班窗口
// 如果顾客休息时间来，就不让顾客入队
// 在输出的时候不用顾客的id而是用，此时队列的长度
#include <cstdlib>
#include <ctime>
#include <iostream>
#include <queue>
...
int main()
{
    setSeed(-1);
    Bank bank;
    int number_customers_perday[8];// 每天的顾客人数
    int Aver_Staytime_perday[8];
    int Sum_time_perday[8];
    for(int week = 1;week < 7; week++){
        OpenForDay(&bank); // 开门?
        number_customers_perday[week] = bank.customer_queue.size;
        Clock(&bank); // 时间流逝-> 事件驱动
        Aver_Staytime_perday[week] = CloseForDay(&bank);
        Sum_time_perday[week] = number_customers_perday[week]*Aver_Staytime_perday[week];
    }
    int WeekTime = 0;
    int WeekPer = 0;
    for (int i = 1;i < 7;i++){
        WeekTime += Sum_time_perday[i];
        WeekPer += number_customers_perday[i];
    }
    cout << "Average Staying Time per Customer: " << WeekTime/WeekPer<< endl; // 关门
    return 0;
}
times calculate_time(int now){
    times arr_time;
...
void genCustomers(Bank* bank)
{
    ...
    for(int i = 0; i<=MAX_SIZE && precus_sum < MAX_ARRIVAL_INTERVAL; i++){
        ...
        // while(precus_sum >= 210 && precus_sum <= 330 || max_time >= 210 && max_time <= 330){
        //     if(precus_sum >= 210 && precus_sum <= 330) precus_sum += 1;
        //     else max_time += 1;
        ...
        if(!(customer.arrival_time>210 && customer.arrival_time< 330)) {
            ...
            // if(customer.arrival_time >= 330){
            //     cout << "Customer " << bank->customer_queue.size << " Arrived at " << time_arr.hour << ":" << time_arr.min << " Estimated Service Time is " << customer.service_time << "min" <<endl;
